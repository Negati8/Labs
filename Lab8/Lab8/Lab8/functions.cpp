...
void SelectionSort(double arr[], int32_t n, bool ascending) {
    for (int32_t i{}; i < n - 1; ++i) {
        ...
        SwapDouble(&arr[i], &arr[ext]);
    }
}


void Merge(double arr[], int32_t left, int32_t mid, int32_t right, bool ascending) {
    int32_t num1{ mid - left + 1 };
    int32_t num2{ right - mid };
    double* L = new double[num1];
    double* R = new double[num2];
    for (int32_t i{}; i < num1; ++i)
        L[i] = arr[left + i];
    for (int32_t j{}; j < num2; ++j)
        R[j] = arr[mid + 1 + j];
    int32_t i{}, j{}, k{ left };
    while (i < num1 && j < num2) {
        bool condition = (ascending) ?
